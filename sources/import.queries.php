...
$cpt=1;
        //prepare CACHE files
        $cacheFile_name = $_SESSION['settings']['cpassman_dir']."/files/cpassman_cache_".md5(time().mt_rand());
        $cacheFile_name_folder = $cacheFile_name."_folders";
        $cacheFile = fopen($cacheFile_name,"w");
        $cacheFileF = fopen($cacheFile_name_folder,"w");

        //read xml file
    	if (file_exists("'".$_SESSION['settings']['cpassman_url']."/files/".$_POST['file'])."'") {
